...
1. **三维 AI 架构同台竞技（直观对比 LLM、LLM+Jev、LLM+Laya）**：
   - **最左侧 · 纯大模型直连 (End-to-End LLM)**：直接将盘面与候选落位提交给大语言模型（如 Gemini / GPT）进行通用 Prompt 推演。受限于自回归 Token 生成耗时与网络往返（延迟 **~800-1500ms**），步频较慢，极易受下落重力压迫，生动呈现纯 System 2 应对高速物理反射场景的迟滞与局限；
   - **中间 · LLM + Jev (云端混合架构)**：TypeSafe Jev 云端离散分类 System 1 协同 LLM 高层战术。擅长大局观推演与盘面地貌重塑，规划 Back-to-Back 连锁与 T-Spin Double 战术打击，决策耗时稳健（**~180-250ms**）；
   - **最右侧 · LLM + Laya (本地 GPU 硬件加速架构)**：部署于本地服务器，依托 NVIDIA TITAN RTX 硬件加速 ModernBERT 嵌入计算。决策耗时仅 **35-45ms**，以极速下落、极限消行防守与超人类 APM/PPS 见长。
...
3. **极简无死角 UI/UX（参考 React-Bits）**：
   - **`SpotlightCard`**：三方选手卡片表面呈现逼真磨砂微光，光随游标平滑流动。
   - **`CallChip` 状态坞**：顶部指示灯呼吸脉动，显示真实微秒延迟。
   - **`FluidGlass` 侧滑抽屉**：右侧流体弹簧滑入，多重高斯模糊背景，彻底消除滚动条截断与死角。
   - **合并统一速度调节**：从休闲热身 0.8 PPS 到 GPU 极速 MAX 统一调控。
4. **一键中英文双语切换 (`i18n`)**：
   - 顶部快捷按钮 `🌐 EN / 中文` 瞬时切换。
   - 涵盖三方画板、HUD 统计指标、AI 战况解说、结算弹窗、快捷键指南及抽屉表单。
5. **电竞级 AI 赛况解说席**：
   - 基于 Gemini 3.8 Flash High 大模型，实时提取三方攻防、堆叠危险度与连击数据，生成电竞风实况解说。
