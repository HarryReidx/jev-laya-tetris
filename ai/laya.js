...
export class LayaClient {
  constructor({ baseUrl } = {}) {
    this.baseUrl = baseUrl ?? process.env.LAYA_URL ?? 'http://172.24.0.5:8300'
    this.model = process.env.LAYA_MODEL ?? 'multilingual'
    this.path = process.env.LAYA_PATH ?? '/v1/predict'
    this.inFlight = 0
    this.latencies = []
  }
  ...
  /** Same contract as JevClient.ask: resolves { answers, latencyMs, usage, model, device }. */
  ask(state, questions, { timeoutMs = 6_000 } = {}) {
    return new Promise((resolve, reject) => {
      const payload = { state, questions, model: this.model }
            ...
            if (res.statusCode === 200 && json) {
              ...
              if (answers) {
                ...
                return finish(() => resolve({
                  answers,
                  usage: json.result?.usage ?? json.usage ?? { input_tokens: 0, output_tokens: 0 },
                  model: json.result?.model ?? json.model_used ?? this.model,
                  latencyMs: json.latency_ms ?? latencyMs,
                  device: json.device ?? 'gpu',
                }))
              }
            }
            if (res.statusCode === 404 && this.path !== '/v1/systemone') {
              this.path = '/v1/systemone'
              this.inFlight--
              return resolve(this.ask(state, questions, { timeoutMs }))
            }
            finish(() => reject(new Error(`Laya HTTP ${res.statusCode}: ${json?.detail ?? json?.status ?? 'bad response'}`)))
          })
        }
      )
      ...
      req.on('error', (err) => {
        const msg = err.code === 'ECONNREFUSED'
          ? `Laya service not running at ${this.baseUrl}`
          : err.message
